...
self.addEventListener('fetch', (event) => {
  ...
  app.get('/s/:id', async (req, res) => {
    try {
      ...
      // ⚠️ 2025.11.06: Featured 페이지일 때 뒤로가기 버튼 자동 주입
      // 추천 갤러리에서 열린 페이지는 앱 내에서 보관함으로 돌아갈 수 있어야 함
      if (page.featured) {
        const appOrigin = `${req.protocol}://${req.get('host')}`;
        const backButtonHTML = `
        <!-- 🔙 추천 갤러리 전용 리턴 버튼 (자동 주입) -->
        <button onclick="window.location.href='${appOrigin}/#archive'" style="position: fixed; top: 1rem; left: 1rem; z-index: 99999; width: 3rem; height: 3rem; display: flex; align-items: center; justify-content: center; border-radius: 9999px; background: rgba(255, 255, 255, 0.95); color: #4285F4; border: none; cursor: pointer; box-shadow: 0 4px 16px rgba(0, 0, 0, 0.2); transition: all 0.3s;" aria-label="보관함으로 돌아가기" data-testid="button-back-to-archive">
        ...
        // <body> 태그 바로 다음에 버튼 삽입
        htmlContent = htmlContent.replace(/<body[^>]*>/, (match) => match + backButtonHTML);
      }
