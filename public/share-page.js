...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        document
            ...
            .setAttribute(
                "content",
                `${linkName}을(를) 통해 새로운 여행 경험을 시작해보세요. 여러분의 눈과 귀가 되어줄 '손안에 가이드'가 지금 바로 당신을 기다립니다.`,
            );
        document
            .getElementById("og-title")
            .setAttribute("content", `${linkName} - 손안에 가이드`);
        document
            .getElementById("og-description")
            .setAttribute(
                "content",
                `${linkName}을(를) 통해 새로운 여행 경험을 시작해보세요.`,
            );
        document
        ...
        try {
            ...
            if (cachedData) {
                ...
                console.log(
                    "📦 오프라인 모드: 로컬스토리지에서 데이터 복구:",
                    shareId,
                );

                // 타이틀과 설명 설정
                descriptionEl.textContent =
                    shareData.name || "공유된 가이드북 (오프라인)";
        ...
        // 로컬스토리지에서도 복구 실패
        showError(
            `가이드북을 불러오는 중 오류가 발생했습니다: ${error.message}`,
        );
    }
});
...
function updateAudioButton(state) {
    ...
    const playIcon =
        '<svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" viewBox="0 0 24 24" fill="currentColor"><path fill-rule="evenodd" d="M4.5 5.653c0-1.426 1.529-2.33 2.779-1.643l11.54 6.648c1.295.748 1.295 2.538 0 3.286L7.279 20.99c-1.25.717-2.779-.217-2.779-1.643V5.653z" clip-rule="evenodd" /></svg>';
    const pauseIcon =
        '<svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" viewBox="0 0 24 24" fill="currentColor"><path fill-rule="evenodd" d="M6.75 5.25a.75 .75 0 01.75-.75H9a.75 .75 0 01.75.75v13.5a.75 .75 0 01-.75.75H7.5a.75 .75 0 01-.75-.75V5.25zm7.5 0a.75 .75 0 01.75-.75h1.5a.75 .75 0 01.75.75v13.5a.75 .75 0 01-.75.75h-1.5a.75 .75 0 01-.75-.75V5.25z" clip-rule="evenodd" /></svg>';
    const loadingIcon =
        '<div class="w-8 h-8 rounded-full animate-spin loader-blue"></div>';
...
function populateShareDetailPage(item) {
    ...
    const shareDescriptionText = document.getElementById(
    ...
    if (shareTextToggleBtn) {
        shareTextToggleBtn.addEventListener("click", () => {
            ...
            if (textOverlay) {
                ...
                console.log(
                    "Text overlay toggled, hidden:",
                    textOverlay.classList.contains("hidden"),
                );
            } else {
