...
exports.handler = async function(event) {
  ...
  try {
    ...
    if (event.httpMethod === "GET") {
      // id 또는 guidebook_id 파라미터 모두 허용
      const guidebookId = event.queryStringParameters.id || event.queryStringParameters.guidebook_id;
      if (!guidebookId) {
        return { statusCode: 400, headers: { "Content-Type": "text/html; charset=utf-8" }, body: "<h1>오류</h1><p>가이드북 ID가 필요합니다. (id 또는 guidebook_id)</p>" };
      }
      // 데이터 조회
      const guidebookData = await store.get(guidebookId, { type: "json" });
      if (!guidebookData) {
        console.error(`[Blobs] guidebookId '${guidebookId}' not found in production store.`);
        return { statusCode: 404, headers: { "Content-Type": "text/html; charset=utf-8" }, body: `<h1>404 - 찾을 수 없음</h1><p>해당 가이드북(${guidebookId})을 찾을 수 없습니다.</p>` };
      }
